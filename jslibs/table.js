...
var Sort = (function(){
		    ...
		    // DD/MM/YYYY HH:MM
		    ,{ re:/(\d{2})\/(\d{2})\/(\d{4}) (\d{2}):(\d{2})/ , f:function(x){ return (new Date(sort.date.fixYear(x[3]),+x[2],+x[1],+x[4],+x[5])).getTime(); } }
...
var Table = (function(){
			 ...
			 if (o.nodeName.toLowerCase()==a || (b && o.nodeName.toLowerCase()==b)) {
			     return o;
			 }
			 while (o=o.parentNode) {
			     if (o.nodeName && (o.nodeName.toLowerCase()==a || (b && o.nodeName.toLowerCase()==b))) {
				 return o;
			     }
			 }
		     }
		     ...
		     if (o!=null && o.nodeName && o.nodeName.toLowerCase()!="table") {
			 o = getParent(o,"table");
		     }
			 ...
			 if (filters.nodeName.toLowerCase()=="select" && filters.type=="select-one" && filters.selectedIndex>-1) {
			     filters={ 'filter':filters.options[filters.selectedIndex].value };
			 }
			 // Also allow for a regular input
			 if (filters.nodeName.toLowerCase()=="input" && filters.type=="text") {
                             // ESBENA: removed ^ from the regex, we want to do full text search
			 ...
			 for (var i=0,L=filters.length; i<L; i++) {
			     ...
			     if (typeof(filter.filter)=="string") {
                                 // ESBENA: make the regex case insensitive
		     ...
		     var f=[removeClass,addClass];
                     // ESBENA: changed the functions to alternate between 'even' and 'odd', no other stripeclasses will be used! 
                     var f=[function(cRow, className){removeClass(cRow,'odd');addClass(cRow,'even')},
                            function(cRow, className){removeClass(cRow,'even');addClass(cRow,'odd')}]
		     for (var i=0,L=bodies.length; i<L; i++) {
		     ...
		     table.processTableCells(t, "thead", function(cell) {
				                     ...
				                     if (colValues.length>0) {
                                                         if(cell.innerHTML == ""){ // ESBENA: avoiding double filters (ACTUAL BUG!)
					                     if (typeof(args.insert)=="function") {
						                 func.insert(cell,colValues);
					                     }
                                                             else{
                                                                 cell.innerHTML += "<input class='filterField' type='text' onkeyup='Table.filter(this,this)' />";
                                                             }
                                                         }
                                                         // ESBENA: changed to have a regex filter as the default.
					                 // else {
